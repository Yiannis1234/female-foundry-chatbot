// WIX PREVIEW WORKING VERSION - This should work in preview mode
...
$w.onReady(function () {
    ...
    // Initialize chatbot - wait for everything to be ready
    console.log('=== WIX CODE LOADED ===');
    
    // TEST: Try to create a simple test button first
    setTimeout(() => {
        ...
        try {
            if (document && document.body) {
                ...
                testBtn.style.cssText = 'position:fixed;top:10px;right:10px;background:red;color:white;padding:20px;z-index:999999;font-size:20px;';
                document.body.appendChild(testBtn);
                console.log('TEST: Test button created!', testBtn);
                
                // If test button works, try chatbot
                setTimeout(() => {
                    console.log('TEST: Test button visible, now trying chatbot...');
                    initChatbot();
                }, 500);
            } else {
                console.error('TEST: document.body not available');
            }
        } catch (e) {
            console.error('TEST: Error creating test button:', e);
        }
    }, 1000);
    
    // Use multiple methods to ensure it runs
    if (typeof window !== 'undefined') {
        // Method 1: Try immediately
        if (document && document.body) {
            console.log('Method 1: Immediate');
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => {
                console.log('Method 2: DOMContentLoaded');
                setTimeout(() => initChatbot(), 1500);
            });
        } else {
            console.log('Method 2: Already loaded');
            setTimeout(() => initChatbot(), 1500);
        }
        
        // Method 3: Wait for window load
        window.addEventListener('load', () => {
            console.log('Method 3: Window load');
            setTimeout(() => initChatbot(), 2000);
        });
        
        // Method 4: Multiple retries
        let attempts = 0;
        const maxAttempts = 15;
        const retryInterval = setInterval(() => {
            attempts++;
            console.log(`Method 4: Retry attempt ${attempts}`);
            const result = initChatbot();
...
function initChatbot() {
    console.log('initChatbot called - checking environment...');
    console.log('window exists?', typeof window !== 'undefined');
    console.log('document exists?', typeof document !== 'undefined');
    console.log('document.body exists?', document && document.body ? 'YES' : 'NO');
    ...
    if (document.getElementById('ff-chatbot-launcher')) {
        ...
        const existingBtn = document.getElementById('ff-chatbot-launcher');
        console.log('Existing button:', existingBtn);
        console.log('Existing button computed style:', window.getComputedStyle(existingBtn).display);
        return true;
    }
    
    console.log('Environment OK, creating chatbot...');
    ...
    try {
        ...
        if (btn) {
            console.log('Button found after creation!', btn);
        } else {
        ...
        if (btn) {
            ...
            `;
            console.log('Button styles forced immediately!');
        ...
        setTimeout(() => {
            const btn2 = document.getElementById('ff-chatbot-launcher');
            if (btn2) {
                btn2.style.cssText = `
                    position: fixed !important;
                    bottom: 24px !important;
                    ...
                    pointer-events: auto !important;
                `;
                console.log('Button styles forced again after delay!');
            }
        }, 500);
